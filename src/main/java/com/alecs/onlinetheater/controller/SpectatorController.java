...
public class SpectatorController {
    ...
    @GetMapping("/{spectatorId}/play")
    public ResponseEntity<List<Play>> listWatchedPlays(@PathVariable int spectatorId) {
        return ResponseEntity.ok().body(spectatorService.getSpectatorPlays(spectatorId));
    }

    @ApiOperation(value = "Add a new spectator ", response = ResponseEntity.class)
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "Successfully added the spectator"),
    ...
    public ResponseEntity<Spectator> addSpectator (@RequestBody @Valid Spectator spectator) {
        ...
        return ResponseEntity.created(URI.create("/spectator/" + savedSpectator.getSpectatorId()))
                .body(savedSpectator);
    }
    ...
    public ResponseEntity<String> deleteSpectator (@PathVariable int spectatorId) {
        if (spectatorService.deleteSpectator(spectatorId) != -1) {
            return ResponseEntity.ok().body("Spectator with the id: " + spectatorId + " was successfully deleted!");
        }
        return ResponseEntity.notFound().build();
    }
}
