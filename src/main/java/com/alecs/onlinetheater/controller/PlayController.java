...
public class PlayController {
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "Successfully added the play"),
    ...
    public ResponseEntity<Play> addPlay(@RequestBody @Valid Play play, @RequestParam int roomId, @RequestParam int playwrightId) {
        Play savedPlay = playService.addNewPlay(play, roomId, playwrightId);
        return ResponseEntity.created(URI.create("/play/" + savedPlay.getPlayId())).body(savedPlay);
    }
